...
export default function EditExperienceClient({ slug }: { slug: string }) {
  ...
  if ((query.error as AxiosError | null)?.response?.status === 404) {
    return <div className="border border-border p-8"><h1 className="font-serif text-4xl">Experience not found</h1><Button className="mt-6" href={listUrl} externalIcon={false}>Back to Experiences</Button></div>;
  }
  ...
  return (
    <>
      ...
      <ExperienceForm mode="edit" initialExperience={query.data.data} isSubmitting={mutation.isPending} submitError={submitError} onSubmit={onSubmit} onCancel={() => router.push(detailUrl)} />
    </>
  );
}
