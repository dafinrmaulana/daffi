...
export default function EditProjectClient({ slug }: { slug: string }) {
  ...
  if ((query.error as AxiosError | null)?.response?.status === 404) return <div className="border border-border p-8"><h1 className="font-serif text-4xl">Project not found</h1><Button className="mt-6" href={listUrl} externalIcon={false}>Back to Projects</Button></div>;
  ...
  return <><AdminPageHeader eyebrow="Portfolio" title="Edit Project" /><ProjectForm mode="edit" initialProject={query.data.data} isSubmitting={mutation.isPending} submitError={submitError} onSubmit={onSubmit} onCancel={() => router.push(detailUrl)} /></>;
}
